...
def _meta_sort_key(meta):
    published = str(meta.get("file_published_date") or "")
    quarter = _int_or_none(meta.get("file_quarter")) or 0
    title = str(meta.get("file_title") or "")
    return (published, quarter, title)


def _meta_matches_id(meta, document_id):
    if not document_id:
        return True

    needle = str(document_id).strip().lower()
    haystacks = [
        str(meta.get("file_title") or "").lower(),
        str(meta.get("file_name_original") or "").lower(),
        str(meta.get("permalink") or "").lower(),
        str(meta.get("link_url") or "").lower(),
        str(meta.get("file_url") or "").lower(),
    ]
    return any(needle == value or needle in value for value in haystacks)


def _select_release_metas(metas, args):
    selected = [meta for meta in metas if is_release_pdf(meta)]

    if args.quarter is not None:
        selected = [meta for meta in selected if _int_or_none(meta.get("file_quarter")) == args.quarter]

    if args.document_id:
        selected = [meta for meta in selected if _meta_matches_id(meta, args.document_id)]

    selected = sorted(selected, key=_meta_sort_key, reverse=bool(args.recent_only))

    if args.one_per_year:
        if selected:
            selected = [max(selected, key=_meta_sort_key)]
...
def process_year(
    ...
    release_metas = _select_release_metas(metas, args)

    print(
        f"[INFO] Ano={year} documentos selecionados={len(release_metas)} "
        f"(candidatos={len(metas)})"
    )
    for meta in release_metas:
        print(
            f"[INFO] Selecionado: ano={year} trimestre={meta.get('file_quarter')} "
            f"titulo={meta.get('file_title')}"
        )
...
def parse_args():
    ...
    parser.add_argument("--year", type=int, help="Processa apenas um ano dentro do intervalo informado")
    parser.add_argument("--quarter", type=int, choices=[1, 2, 3, 4], help="Processa apenas um trimestre")
    parser.add_argument("--document-id", type=str, help="Filtra documentos por título, URL ou caminho")
    parser.add_argument("--limit", type=int, help="Limita a quantidade de documentos baixados por execução")
    parser.add_argument("--one-per-year", action="store_true", help="Seleciona apenas um release por ano")
    parser.add_argument("--recent-only", action="store_true", help="Prioriza os releases mais recentes")
